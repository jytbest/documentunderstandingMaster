...
public class FileController extends BaseController {
    ...
    public CommonReturnType updatelabel(@RequestParam(name="fileid") Integer id) throws BusinessException {
        ...
        fileService.updatelabel(fileModel);
        if(fileModel.getLabel() ==null){
    ...
    public CommonReturnType updatetxt(@RequestParam(name="txtfile") MultipartFile txt,
        ...
        //获取到txt文件，然后保存到指定路径
        String rootpath = "...../resources/file/txt";
        //获取文件名（包括后缀）
        String filename = txt.getOriginalFilename();
        //文件目录
        String path = rootpath+"/"+filename;
        //调簇分类器需要的路径
        String grupath = "documentunderstanding/gru/"+label+"/test/test.txt";
        FileOutputStream fos = null;
        FileOutputStream pfos = null;
        try{
            fos = new FileOutputStream(path);
            ...
            try {
                ...
                fileModel.setFileid(id);
                fileService.updatetxt(fileModel);
            } catch (IOException e) {
                throw new BusinessException(EmBusinessError.TXT_FAIL_UPDATE);
            }
        }
    ...
    @ResponseBody
    public CommonReturnType updatereport(@RequestParam(name="fileid") Integer id) throws BusinessException {
        ...
        FileModel fileModel = new FileModel();
        fileModel.setNewdocx(path);
        fileModel.setFileid(id);
        fileService.updatenewdocx(fileModel);
        if(fileModel.getNewdocx() ==null){
