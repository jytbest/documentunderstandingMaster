...
def predict(path):
    ...
    label_dic = {0: '未知', 1: '标志码', 2: '封面标题', 3: '版本信息', 4: '题目', 5: '文本段', 6: '作者单位', 7: '日期', 8: '前言', 9: '表题内容',
                 10: '表格', 11: '正文标题', 12: '图题编号', 13: '图题内容', 14: '表题编号', 15: '副题目', 16: '图片'}

    y_predict = y_predict.astype(int)

    predict_list = []    #保存实际标签结果
    for x in y_predict:
        for i in x:
    ...
    dataFrame.to_csv("output/compareResult-predict.csv",index = False,encoding ='utf-8')
...
if __name__ == '__main__':
    predict('./newinput/BZprediction.csv')
